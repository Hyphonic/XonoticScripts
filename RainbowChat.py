...
import colour, pyperclip# Install with < pip install colour pyperclip rich >
from rich import *
...
ShowColors = True # Show the colors in the console
...
with open('Colors.txt', 'r') as File: # All the available colors that can be used to convert the message.
    ColorNames = File.read().splitlines()
    Count = 0
    for Color in Colors.values():
        if ShowColors:
            Console.print(ColorNames[Count], style='rgb'+str(list(Colors)[list(Colors.values()).index(Color)]).replace(' ', ''))
            Count += 1
del Count
'''
with open('Colors.txt', 'r') as File: # All the available colors that can be used to convert the message.
...
try:
    while True:
        ...
        Message = input(' > ')
        Red = colour.Color('FireBrick') # The first color in the range.
        Green = colour.Color('DarkViolet') # The last color in the range.
        Ranges = list(Red.range_to(Green, len(Message)))
        Count = 0
        RangeList = []
        print('The message will look like this: ', end='')
        for Letter in Message:
            try:
                Console.print(Letter, style=Ranges[Count].hex_l, end='')
            except:
                pass
            Count += 1
        print('\nSay ', end='')
        ...
        print('\nThe Converted Message Has Been Copied To The Clipboard!')
